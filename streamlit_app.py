...
# --- Data Loading (with Caching and Arrow fix) ---
@st.cache_data
def load_data(file_path):
    df = pd.read_csv(file_path)
    # Clean column names
    df.columns = df.columns.str.strip()
    # Fix for PyArrow/NumPy Int64 issues
    return df.convert_dtypes()

# Load both datasets
survey_df = load_data("Stress_Dataset.csv")
numeric_df = load_data("StressLevelDataset.csv")

# --- Sidebar Setup ---
st.sidebar.title("Student Stress Analysis 🎓")
# Allowing users to switch between the raw survey data and the cleaned numeric dataset
data_choice = st.sidebar.radio("Select Dataset", ["Survey Data (Qualitative)", "Stress Level Data (Quantitative)"])

if data_choice == "Survey Data (Qualitative)":
    df = survey_df
else:
    df = numeric_df

page = st.sidebar.selectbox("Navigate", ["Introduction 📘", "Visualization 📊", "Automated Report 📑", "Prediction 🤖"])
...
if page == "Introduction 📘":
    ...
    st.write(f"Currently viewing: **{data_choice}**")
...
# --- 02 Visualization ---
    ...
    col_y = st.selectbox("Select Y-axis variable", df.columns, index=min(1, len(df.columns)-1))

    tab1, tab2, tab3 = st.tabs(["Bar Chart 📊", "Line Chart 📈", "Correlation Heatmap 🔥"])

    with tab1:
        st.subheader("Bar Chart")
        # Handling large datasets for bar charts by grouping or limiting
        chart_data = df[[col_x, col_y]].head(100).sort_values(by=col_x)
        st.bar_chart(chart_data.set_index(col_x))

    with tab2:
        st.subheader("Line Chart")
        st.line_chart(df[[col_x, col_y]].head(100).sort_values(by=col_x).set_index(col_x))
...
# --- 03 Automated Report ---
    ...
    st.info("This will generate a comprehensive data profile using YData Profiling.")
    
    if st.button("Generate Report"):
        with st.spinner("Analyzing data..."):
            profile = ProfileReport(df, title="Student Stress Report", explorative=True, minimal=True)
            st_profile_report(profile)
            
            export = profile.to_html()
            st.download_button(label="📥 Download HTML Report", data=export, file_name="stress_analysis_report.html", mime='text/html')

# --- 04 Prediction ---
elif page == "Prediction 🤖":
    st.subheader("04 Prediction with Linear Regression")
    
    # We use the numeric dataset for prediction as it's better suited for regression
    predict_df = numeric_df.copy().dropna()
    
    # Preprocessing: Encode any categorical columns if they exist
    le = LabelEncoder()
    for col in predict_df.select_dtypes(include=['object']).columns:
        predict_df[col] = le.fit_transform(predict_df[col])

    list_var = list(predict_df.columns)
    
    st.sidebar.markdown("---")
    st.sidebar.subheader("Model Settings")
    
    # Defaulting to 'stress_level' if available
    default_target = "stress_level" if "stress_level" in list_var else list_var[-1]
    
    target_selection = st.sidebar.selectbox("Select Target Variable (Y)", list_var, index=list_var.index(default_target))
    
    # Default features excluding the target
    default_features = [c for c in list_var if c != target_selection]
    ...
    if features_selection:
        X = predict_df[features_selection]
        y = predict_df[target_selection]
        ...
        st.success(f"Model Training Complete! Average Error (MAE): {np.round(mae, 2)}")

        # Visualizing Results
        fig, ax = plt.subplots()
        ax.scatter(y_test, predictions, alpha=0.5, color='purple')
